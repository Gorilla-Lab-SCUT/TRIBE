...
class BasicBlock(nn.Module):
    ...
    def forward(self, data):
        ...
        if not self.equalInOut:
            x = self.relu1(self.bn1(x, y))
        else:
            out = self.relu1(self.bn1(x, y))
        out = self.relu2(self.bn2(self.conv1(out if self.equalInOut else x), y))
        if self.droprate > 0:
            out = F.dropout(out, p=self.droprate, training=self.training)
        out = self.conv2(out)
        return (torch.add(x if self.equalInOut else self.convShortcut(x), out), y)
...
class NetworkBlock(nn.Module):
    ...
    def forward(self, x, y):
        return self.layer((x, y))[0]


class ModifiedWideResNet(nn.Module):
    ...
    def __init__(self, depth=28, num_classes=10, widen_factor=10, sub_block1=False, dropRate=0.0, bias_last=True):
        super(ModifiedWideResNet, self).__init__()
