...
def load_model_bayes(model_name: str,
    ...
    if "_swa" in model_name:
        model_name = model_name[:-4]
    swa_model_name = model_name+"_swa"
    cov_model_name = model_name+"_cov"
    swa_model_path = model_dir_ / f'{swa_model_name}.pt'
    cov_model_path = model_dir_ / f'{cov_model_name}.pt'

    if not isinstance(models[model_name]['gdrive_id'], list):
        model = models[model_name]['model']()
        model_swa = models[model_name]['model']()
        model_cov = models[model_name]['model']()
        if dataset_ == BenchmarkDataset.imagenet and 'Standard' in model_name: ## why return after this??? ##
            return model.eval()
        ...
        if 'Kireev2021Effectiveness' in model_name or model_name == 'Andriushchenko2020Understanding':
            ...
            checkpoint_swa = checkpoint_swa['last']  # we take the last model (choices: 'last', 'best')
            checkpoint_cov = checkpoint_cov['last']  # we take the last model (choices: 'last', 'best')
        try:
        ...
        model_swa = _safe_load_state_dict(model_swa, model_name, state_dict_swa, dataset_)
        model_cov = _safe_load_state_dict(model_cov, model_name, state_dict_cov, dataset_)

        return model.eval(), model_swa.eval(), model_cov.eval()
    ...
    # If we have an ensemble of models (e.g., Chen2020Adversarial)
        ...
        for i, gid in enumerate(models[model_name]['gdrive_id']):
            ...
            checkpoint_swa = torch.load('{}_m{}.pt'.format(swa_model_path, i),
                                    map_location=torch.device('cpu'))
            checkpoint_cov = torch.load('{}_m{}.pt'.format(cov_model_path, i),
                                    map_location=torch.device('cpu'))
            try:
                state_dict = rm_substr_from_state_dict(
                    checkpoint['state_dict'], 'module.')
                state_dict_swa = rm_substr_from_state_dict(
                    checkpoint_swa['state_dict'], 'module.')
                state_dict_cov = rm_substr_from_state_dict(
                    checkpoint_cov['state_dict'], 'module.')
            except KeyError:
                state_dict = rm_substr_from_state_dict(checkpoint, 'module.')
                state_dict_swa = rm_substr_from_state_dict(checkpoint_swa, 'module.')
                state_dict_cov = rm_substr_from_state_dict(checkpoint_cov, 'module.')
            ...
            model.models[i] = _safe_load_state_dict(model.models[i],
                                                    ...
                                                    dataset_)
            model_swa.models[i] = _safe_load_state_dict(model_swa.models[i],
                                                    model_name, state_dict_swa,
                                                    dataset_)
            model_cov.models[i] = _safe_load_state_dict(model_cov.models[i],
                                                    model_name, state_dict_cov,
                                                    dataset_)
            model.models[i].eval()
            model_swa.models[i].eval()
            model_cov.models[i].eval()
...
def _safe_load_state_dict(model: nn.Module, model_name: str,
    ...
    known_failing_models = {
        ...
        "ModifiedHendrycks2020AugMix_ResNeXt",
