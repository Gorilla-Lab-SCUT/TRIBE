...
class AvgResultProcessor:
    def __init__(self, label2name: dict):
        ...
        for i in range(self.num_class):
            key = list(self.label2name.keys())[i]
            self.acc_per_class[key] = 0
            self.count_per_class[key] = 0
    ...
    def calculate(self):
        for c in range(self.num_class):
            key = list(self.label2name.keys())[c]
            self.result_per_class[key] = self.acc_per_class[key] / self.count_per_class[key] if self.count_per_class[key] > 0 else 0.0

    def info(self):
        t = PrettyTable(["Corruption", "Accuracy", "Error Rate"])
        for c in range(self.num_class):
            if c > len(self.label2name.keys()):
                continue
            key = list(self.label2name.keys())[c]
            t.add_row([self.label2name[key], f"{self.result_per_class[key] * 100:.2f}", f"{(1 - self.result_per_class[key]) * 100:.2f}"])
