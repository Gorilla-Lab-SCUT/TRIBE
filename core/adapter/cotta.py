...
class CoTTA(nn.Module):
    ...
    def __init__(self, cfg, model, optimizer):
        ...
        self.steps = self.cfg.ADAPTER.COTTA.STEPS
        assert self.steps > 0, "cotta requires >= 1 step(s) to forward and update"
        self.episodic = self.cfg.ADAPTER.COTTA.EPISODIC
        
        self.model_state, self.optimizer_state, self.model_ema, self.model_anchor = \
            copy_model_and_optimizer(self.model, self.optimizer)
        self.transform = get_tta_transforms()    
        self.mt = self.cfg.ADAPTER.COTTA.MT_ALPHA
        self.rst = self.cfg.ADAPTER.COTTA.RST_M
        self.ap = self.cfg.ADAPTER.COTTA.AP
